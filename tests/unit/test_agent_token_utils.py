...
async def test_get_client_credentials_token_success(mock_http_client):
    ...
    mock_post.json.return_value = {"access_token": "token123", "token_type": "bearer", "expires_in": 3600}
    ...
    assert result == {"access_token": "token123", "token_type": "bearer", "expires_in": 3600}
...
async def test_get_agent_token_from_credentials_file_success(mock_get_token, mock_load):
    credentials = AgentCredentials(client_id="id1", client_secret="secret1", scopes=["s1"])
...
@patch("bindu.utils.agent_token_utils.load_agent_credentials")
@patch("bindu.utils.agent_token_utils.get_client_credentials_token")
async def test_get_agent_token_from_credentials_file_no_token(mock_get_token, mock_load):
    credentials = AgentCredentials(client_id="id1", client_secret="secret1", scopes=["s1"])
    mock_load.return_value = credentials
